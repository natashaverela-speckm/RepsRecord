...
// ── READ PATH (C-1/C-2): merge, never replace ───────────────────────────────
// Deliberately not a hard cutover. Load the blob as before, then merge attested
// rows over it: table wins on shared entries and supplies the server createdAt;
// table-only entries are adopted; local-only entries are kept; a fetch failure
// changes nothing. Nothing is ever deleted here.
function _rowToEntry(r){
  return{
    id:r.legacy_id, createdAt:r.created_at, date:r.activity_date,
    propertyId:r.legacy_property_id||'', trackType:r.track_type, type:r.track_type,
    category:r.category, hours:Number(r.hours), notes:r.notes||'',
  ...
  try{
    ...
    rows.forEach(r=>{
      ...
      if(local.has(r.legacy_id)){
        local.set(r.legacy_id,Object.assign({},local.get(r.legacy_id),merged));
        updated++;
      }else{
        local.set(r.legacy_id,merged); adopted++;
      }
...
function avgFlag(p,style){
  ...
  return style==='badge'
    ...
    : `<span style="font-size:10.5px;color:#B45309;font-weight:700;">\u26A0 unsubstantiated</span>`;
}
...
<div class="card" style="border:1.5px solid #0D1F3C;margin-bottom:14px;">
  ...
  <div style="margin-top:12px;padding:11px 13px;border-radius:8px;background:${_att.unattested===0?'#F0FDFA':'#FFFBEB'};border:1px solid ${_att.unattested===0?'#99F6E4':'#FDE68A'};">
    ...
    ${_att.unattested>0?`<div style="margin-top:8px;font-size:11px;color:#92400E;line-height:1.6;">\u26A0 Entries without a server timestamp predate this feature or were created offline. They remain valid records, but their creation date rests on this device rather than on independent corroboration.</div>`:''}
  </div>
</div>
...
<div class="card card-mb">
  ...
  <div class="field">
    ...
    ${expandToggle('About the NIIT and your rental income','The 3.8% Net Investment Income Tax applies to the lesser of your net investment income or MAGI above your threshold. REPS status alone does not remove NIIT — rental income is presumptively net investment income under §1411(c)(2)(A). Relief generally requires the rental to rise to a §162 trade or business, or the safe harbor at Reg. §1.1411-4(g)(7), which needs more than 500 hours in rental activities. IRC §1411.')}
  </div>
</div>
...
<div class="banner bn-teal" style="margin-bottom:16px;"><div class="bn-ic">⚖️</div><div><div class="bn-title">Why REPS matters</div><div class="bn-sub"><strong>The baseline without REPS.</strong> Rental real estate is passive by default, and passive losses generally offset only passive income — but not entirely. Under <strong>IRC §469(i)</strong>, a taxpayer who <em>actively participates</em> may deduct up to <strong>$25,000</strong> of rental losses against ordinary income. That allowance phases out between $100,000 and $150,000 of modified AGI and is gone above $150,000.<br><br><strong>What REPS changes.</strong> Qualifying as a real estate professional removes the automatic-passive rule entirely — no $25,000 ceiling, no income phase-out. Where you also materially participate in the rental (see below), the losses become non-passive and can offset W-2, business, and other active income, subject to the at-risk, basis, and §461(l) limits described further down.</div></div></div>
...
<h2 class="sec-lbl">Grouping Election — <span style="text-transform:none;font-family:ui-monospace,monospace;letter-spacing:0;">Reg. §1.469-9(g)</span></h2>
<div class="card card-mb">
...
function toggleRemy(){
  ...
  if(remyOpen){
    ...
    if(remyMessages.length===0){
      ...
      const rh=Math.round((ctx.rh||0)*100)/100;   // exact to 2dp; never round hours in a tax record
  ...
  return{
    ...
    properties:state.properties.map(p=>{
      ...
      return{
        ...
        avgRentalDays:(()=>{const a=avgRentalInfo(p);return a.days;})(),
        avgRentalSource:(()=>{const a=avgRentalInfo(p);return a.source;})(),        // 'computed' | 'manual' | 'none'
        avgRentalSubstantiated:(()=>{const a=avgRentalInfo(p);return a.substantiated;})(),
